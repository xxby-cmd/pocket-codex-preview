...
- 静态启动页在 `launcher/`，支持跳转 GitHub 登录后启动环境，也支持个人访问令牌直接调用 GitHub 的启动、状态、停止 API。默认令牌只在内存保留；勾选保存后存于手机浏览器 localStorage，可清除。不能把令牌提交到仓库。
...
手机收藏 GitHub Pages 入口。首次填入仅限本项目的 GitHub 令牌，选择保存到自己的手机及自动启动。启动成功且 `/ready` 可访问后进入对话。停止返回入口时不自动重启，下次重新访问才启动。云端启动脚本使用官方 GitHub CLI 自动公开 8787；失败会输出明确错误。
