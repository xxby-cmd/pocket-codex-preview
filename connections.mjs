...
export class Connections {
  ...
  async execute(op,a={}){
    ...
    if(op==='send'){
      ...
      if(this.writers.has(a.threadId))throw Error('手机任务正在运行，请等待完成或先停止');
      const w=this.factory();this.writers.set(a.threadId,w);this.leases.set(a.threadId,Date.now());
      w.onCompleted=()=>{setTimeout(()=>{if(this.writers.get(a.threadId)===w){w.close();this.writers.delete(a.threadId);}},0);};
      try{await w.start();const result=await w.execute(op,a);this.sent.set(a.requestId,result);if(this.sent.size>1000)this.sent.delete(this.sent.keys().next().value);if(w.leaving)await this.release(a.threadId);return result;}
    ...
    if(op==='new'){const w=this.factory();try{await w.start();return await w.execute(op,a);}finally{w.close();}}
    const result=await this.reader.execute(op,a);if(op==='read')result.status=this.writers.has(a.threadId)?'working':'unknown';return result;
  }
